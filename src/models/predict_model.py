...
def inference_implicit_model(data_path: str, model_path: str, path_to_save: str) -> None:
	logger = logging.getLogger(__name__)
	model = CosineRecommender(K=20)
	model = model.load(model_path)
	logger.info(f'model loaded from {model_path}')
	interactions_train_matrix = sp.load_npz(os.path.join(data_path, 'interactions_matrix.npz')) 
	...
	recs_mapper = generate_implicit_recs_mapper(model=model, 
												train_matrix=interactions_train_matrix.tocsr(), 
												top_N=10,
												user_mapping=user_mapping,
												item_inv_mapping=item_inv_mapping)
	
	train_data = pd.read_csv("/home/umbilnm/RecSysPlayground/data/interim/one_week_train.csv")
	users = pd.Series(train_data['user_id'].unique()).to_frame(name="user_id").astype(str)
