...
class Evaluator:
    ...
    def _calc_avg_precision_sample(self, true_sample: List[int], predicted_sample: List[int], k: int) -> float:
        score = 0
        last_recall = 0
        for k in range(min(len(predicted_sample)), k):
            prec_at_k = self._calc_precision_sample(true_sample, predicted_sample, k)
            recall_at_k = self._calc_recall_sample(true_sample, predicted_sample, k)
            score += prec_at_k*(recall_at_k-last_recall)
            last_recall = recall_at_k
        return score
